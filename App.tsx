...
const SortButton: React.FC<{
  ...
  onClick: () => void;
  ariaLabel: string;
}> = ({ label, column, activeColumn, direction, onClick, ariaLabel }) => {
    ...
    if (library.length === 0) {
      const loadSampleBooks = async () => {
        ...
        try {
          ...
          setSamplesError(t('popularBooksError'));
        } finally {
  ...
  const renderEmptyState = () => {
    if (isLoadingSamples) {
      return (
        <div className="flex-1 flex flex-col items-center justify-center text-center text-neutral-500 h-full mt-16">
          ...
          <h2 className="text-2xl font-bold mt-4">{t('emptyStateLoading')}</h2>
          <p className="mt-2">{t('emptyStateLoadingSubtitle')}</p>
        </div>
      );
    }
      ...
      return (
        <>
            <div className="text-center mb-6 mt-8">
                <h2 className="text-3xl font-bold text-neutral-300">{t('emptyStateTitle')}</h2>
                <p className="text-neutral-400 mt-1">{t('emptyStateSubtitle')}</p>
            </div>
    ...
    return (
        <div className="flex-1 flex flex-col items-center justify-center text-center text-neutral-500 h-full mt-16">
            ...
            <h2 className="text-2xl font-bold mt-4">{t('emptyStateFallbackTitle')}</h2>
            <p className="mt-2">{samplesError || t('emptyStateFallbackSubtitle')}</p>
          </div>
    );
  }
  ...
  return (
    <main className="bg-neutral-900 text-neutral-200 min-h-screen w-full flex flex-col items-center p-4 sm:p-8 pb-24 relative">
      ...
      <div className="text-center my-8 z-10">
        <h1 className="text-6xl md:text-7xl font-caveat font-bold text-neutral-100">{t('appTitle')}</h1>
        <p className="text-neutral-400 mt-2 text-lg">{t('appSubtitle')}</p>
      </div>
        ...
        {library.length > 0 ? (
          <>
            <div className="flex items-center justify-end gap-2 mb-4 px-1">
              <span className="text-neutral-400 text-sm mr-2">{t('sortBy')}</span>
              <SortButton label={t('sortDate')} column="added" activeColumn={sortBy} direction={sortDirection} onClick={() => handleSortChange('added')} ariaLabel={`Sort by Date Added in ${sortBy === 'added' && sortDirection === 'asc' ? 'descending' : 'ascending'} order`} />
              <SortButton label={t('sortTitle')} column="title" activeColumn={sortBy} direction={sortDirection} onClick={() => handleSortChange('title')} ariaLabel={`Sort by Title in ${sortBy === 'title' && sortDirection === 'asc' ? 'descending' : 'ascending'} order`} />
              <SortButton label={t('sortAuthor')} column="author" activeColumn={sortBy} direction={sortDirection} onClick={() => handleSortChange('author')} ariaLabel={`Sort by Author in ${sortBy === 'author' && sortDirection === 'asc' ? 'descending' : 'ascending'} order`} />
            </div>
            <motion.div layout className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-4 sm:gap-6 w-full">
              <AnimatePresence>
